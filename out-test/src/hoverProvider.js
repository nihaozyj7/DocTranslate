...
function createHoverProvider() {
    return vscode.languages.registerHoverProvider({ scheme: 'file' }, {
        ...
        async provideHover(document, position) {
            ...
            try {
                // 获取 VSCode 原生的悬停内容
                const raw = await vscode.commands.executeCommand('vscode.executeHoverProvider', document.uri, position);
                // 如果没有原生悬停内容则返回
                if (!raw || raw.length === 0)
                    return;
                // 提取并整合原始悬停文本
                const original = raw
                    .map(h => h.contents.map(c => c.value ?? String(c)).join('\n'))
                    .join('\n\n');
                // 生成文本的 MD5 哈希值作为缓存键
                ...
                // 检查现有缓存
                const entry = cache_1.translationCache.get(hash);
                const valid = (0, cache_1.isCacheValid)(entry);
                ...
                // 添加翻译控制工具栏
                if ((0, commands_1.getShowTranslated)()) {
                    md.appendMarkdown(`✨ **悬浮文档翻译** &nbsp;&nbsp;👉 ` +
                    ...
                    else if (cache_1.allowShowTranslated.includes(hash) && cache_1.translationCache.has(hash)) {
                        md.appendMarkdown(`[重新翻译](command:hoverTranslator.retranslate?${encodeURIComponent(JSON.stringify([encoded]))})`);
                    }
                    // 用户未点击翻译按钮时，只显示翻译按钮
                    else {
