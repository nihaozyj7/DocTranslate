...
exports.openCacheView = openCacheView;
const vscode = __importStar(require("vscode"));
const cache_1 = require("../cache");
/**
 * 打开缓存可视化 Webview
 */
function openCacheView(context) {
    const panel = vscode.window.createWebviewPanel('hoverTranslatorCache', '翻译缓存管理', vscode.ViewColumn.One, { enableScripts: true });
    const update = () => {
        panel.webview.html = getHtml(Array.from(cache_1.translationCache.entries()));
    };
    update();
    // 监听前端消息
    panel.webview.onDidReceiveMessage(msg => {
        if (msg.type === 'delete') {
            cache_1.translationCache.delete(msg.hash);
            (0, cache_1.saveCache)();
            update();
        }
        if (msg.type === 'clear') {
        ...
        if (msg.type === 'editSave') {
            const entry = cache_1.translationCache.get(msg.hash);
            if (entry) {
                entry.text = msg.text;
                entry.time = Date.now();
                cache_1.translationCache.set(msg.hash, entry);
                (0, cache_1.saveCache)();
            }
            update();
        }
...
//# sourceMappingURL=cacheView.js.map